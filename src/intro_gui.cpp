...
struct SelectGameWindow : public Window {
	...
	{
		/* ---------------------------------------------------------------
		 * Archipelago: pump the network client from here.
		 * TimerGameRealtime only fires in GM_NORMAL — so while we are on
		 * the main menu (GM_MENU) the AP worker thread queues events but
		 * Tick() is never called and the queue never drains.
